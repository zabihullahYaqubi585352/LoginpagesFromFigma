    ...
    <div class="grid grid-cols-12 px-4 w-full pb-32">
      <q-form class="col-span-12 md:col-span-8 md:col-start-3">
        ...
        >
          ...
          >
            <div
              class="flex flex-col items-start justify-center gap-2 w-full px-2"
            >
              <span class="perfectText">perfect</span>
              <div class="w-full h-[9px] rounded-[1000px] bg-[#aeb3af] mb-4">
                <div
                  class="max-w-[725px] w-full h-[9px] rounded-[1000px] bg-[#00962D] mb-4"
                ></div>
              </div>
            </div>
            ...
            <template #append>
              ...
              :rules="rules.password"
              lazy-rules
              :type="state.password_field_type2"
              icon_name="lock"
              ref="password"
              class="py-2"
     ...
     <template #append>
          ...
          <CustomSubmit
            text="Enter"
            @submit="handleLogin"
            :disabled="!state.confirmPassword.trim() || !state.password.trim()"
          />
          ...
          <div
            ...
            class="text-[#ED1C24] font-[Arena_Uno] text-[25px] font-normal text-center mt-8"
          >
...
let state = reactive({
  ...
  visibility_icon2: "visibility",
  visibility: false,
  visibility2: false,
  password_field_type: "password",
  password_field_type2: "password",
   touched: {
...
const switchVisibility2 = () => {
  state.visibility2 = !state.visibility2;
  state.password_field_type2 = state.visibility2 ? "text" : "password";
  state.visibility_icon2 = state.visibility2 ? "visibility_off" : "visibility";
};
const rules = {
  password: [(value) => !!value || this.$t("PasswordRequired")],
};
const handleLogin = () => {
  state.touched.password = true;
  state.touched.confirmPassword = true;

  // Check if each field meets password rules
  state.errors.password = !validatePassword(state.password);
  state.errors.confirmPassword = !validatePassword(state.confirmPassword);

  // 🔥 Check if passwords match
  if (state.password !== state.confirmPassword) {
    state.errors.password = true;
    state.errors.confirmPassword = true;
    state.generalError = true; // show red message
    return;
  }
  ...
  if (!state.errors.password && !state.errors.confirmPassword) {
    ...
    alert("✅ Passwords match! You can proceed.");
  }
};
...
watch(() => state.confirmPassword, (val) => {
  if (state.touched.confirmPassword) {
    state.errors.confirmPassword = !validatePassword(val);
  }
});
